...
use crate::types::{
    LiquidityQuote, PoolInfo, SdkAmmError, SwapInQuote, SwapOutQuote, SwapSimulation,
};
...
impl<'a> AmmPoolSdk<'a> {
    ...
    pub fn execute_swap(
        ...
        unwrap_contract_result(
            self.client
                .try_swap(trader, token_in, &amount_in, &min_out, &deadline),
        )
    }
    ...
    pub fn flash_loan(
        ...
        unwrap_contract_result(
            self.client
                .try_flash_loan(receiver, &amount_a, &amount_b, &data),
        )
    }
}
